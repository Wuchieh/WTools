                    ...
                    <v-card-text class="pt-4">
                        ...
                        >
                            ...
                            >
                                ...
                                <v-list-item-title :style="{ color: textColorFor(c) }">
                                    {{ c }}
                                </v-list-item-title>
                                <template #append>
                                    <v-btn
...
const rgbString = computed(() => {
    const r = Number.parseInt(color.value.slice(1, 3), 16);
    const g = Number.parseInt(color.value.slice(3, 5), 16);
    const b = Number.parseInt(color.value.slice(5, 7), 16);
    return `rgb(${r}, ${g}, ${b})`;
});

const hslString = computed(() => {
    const r = Number.parseInt(color.value.slice(1, 3), 16) / 255;
    const g = Number.parseInt(color.value.slice(3, 5), 16) / 255;
    const b = Number.parseInt(color.value.slice(5, 7), 16) / 255;
...
function copy(text: string) {
    navigator.clipboard.writeText(text).then(() => {
        ...
        if (showCopySnackbar) showCopySnackbar(t('color.copied'));
    }).catch(() => {
        const showCopySnackbar = inject<(text: string, color?: string) => void>('showCopySnackbar');
        if (showCopySnackbar) showCopySnackbar(t('color.copyFailed'), 'error');
    });
}
