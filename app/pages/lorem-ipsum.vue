<template>
    <v-container class="py-10">
        <h1 class="font-weight-bold text-h3 mb-2 text-center">{{ $t('lorem.title') }}</h1>
        <p class="text-body-1 text-medium-emphasis mb-10 text-center">{{ $t('lorem.subtitle') }}</p>
        <v-row justify="center">
            <v-col cols="12" lg="8">
                <v-card border>
                    <v-card-text class="pt-4">
                        <v-select v-model="unit" :items="unitOptions" :label="$t('lorem.unit')" class="mb-4" />
                        <v-slider v-model="count" :label="$t('lorem.count', { v: count })" :min="1" :max="50" thumb-label class="mb-4" />
                        <v-btn color="primary" block @click="generate">{{ $t('lorem.generate') }}</v-btn>
                        <v-textarea v-if="text" :model-value="text" rows="10" border readonly class="mt-4 font-monospace" />
                        <v-btn v-if="text" color="success" block class="mt-2" @click="copy">{{ $t('lorem.copy') }}</v-btn>
                    </v-card-text>
                </v-card>
            </v-col>
        </v-row>
...
const { t } = useI18n();
useHead({ meta: [{ content: t('lorem.subtitle'), name: 'description' }], title: t('lorem.title') });
...
function generate() {
    ...
    if (unit.value === 'words') {
        ...
        result = Array.from({ length: count.value }, (_, i) => {
