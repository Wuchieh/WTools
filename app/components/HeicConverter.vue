<template>
    <v-card border>
            ...
            <v-list v-if="store.files.length > 0" border>
                ...
                >
                    ...
                    <template #append>
                        ...
                        <v-chip v-else-if="f.status === 'converting'" color="warning" size="small" class="mr-2">...</v-chip>
        ...
        <v-card-actions v-if="store.files.length > 0">
            ...
            <v-spacer />
            <v-btn color="primary" :loading="store.isConverting" @click="store.convertFiles()">{{ $t('heic.convert') }}</v-btn>
