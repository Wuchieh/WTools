<template>
    <v-card border>
        ...
        <v-card-text class="pt-4">
            ...
            <v-row class="mb-4">
                <v-col cols="4">
                    <v-select v-model="store.outputFormat" :items="formats" :label="$t('gif.outputFormat')" density="compact" />
                </v-col>
                <v-col cols="4">
                    <v-text-field v-model="store.maxWidth" :label="$t('gif.maxWidth')" type="number" density="compact" clearable :min="1" />
                </v-col>
            </v-row>
        ...
        <v-card-actions v-if="store.files.length > 0">
            ...
            <v-spacer />
            <v-btn color="primary" :loading="store.isConverting" @click="store.convertFiles()">{{ $t('gif.process') }}</v-btn>
